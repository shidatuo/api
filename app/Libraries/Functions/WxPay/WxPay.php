...
class WxPay{
    ...
    public function __construct($cfg){
        WxPayRefundConfig::setConfig($cfg);
    }
    ...
    public function refundOrder($totalFee, $refundFee, $refundNo, $transactionIdOrOutTradeNo, $type = 0){
        ...
        log_ex('debug_pay',"----pay_input:" . print_r($input, true));
        try {
            $result = WxPayApi::refund($input);
            log_ex('debug_pay',"----pay_success:" . print_r($result, true));
            return $result;
        } catch (Exception $e) {
            $this->lastErrMsg = $e->getMessage();
            log_ex('debug_pay',"----pay_lastErrMsg:" . print_r($this->lastErrMsg, true));
            return false;
        }
    }
     ...
     * 企业付款
     * @param $refundFee 金额
     * @param $transactionIdOrOutTradeNo 商户订单号
     * @return bool|array
     */
    public function transfersOpenId($price,$transactionIdOrOutTradeNo,$openid,$check_name,$re_user_name,$user_ip= false,$desc = false){
        $input = new WxPayRefund();
        $input->set_price($price);//金额
        $input->partner_trade_no($transactionIdOrOutTradeNo);//商户订单号
        $input->openid($openid);//用户openid
        $input->check_name($check_name);//用户openid
        $input->re_user_name($re_user_name);//用户openid
        if(!empty($user_ip)){
            $input->spbill_create_ip($user_ip);//用户openid
        }else{
            $input->spbill_create_ip($_SERVER['SERVER_ADDR']);//用户openid
        }
        if(empty($desc)){
            $news = '向用户付款测试';
        }else{
        ...
        $input->desc($news);//企业付款操作说明信息。必填。
        log_ex('transfers'.date('Y-m-d'),"- transfersOpenId params -" . print_r($input, true));
        try {
            $result = WxPayApi::transfers($input);
            log_ex('transfers'.date('Y-m-d'),"- return result -" . print_r($result, true));
            return $result;
        } catch (Exception $e) {
            $this->lastErrMsg = $e->getMessage();
            return false;
     ...
     * @return bool|string
     */
    public function getNativePayUrl2($productId, $body, $out_trade_no, $amount, $notifyUrl,$ip,
    ...
    public function getJsApiPayParams($openId, $body, $out_trade_no, $amount, $notifyUrl,
                                      $attach = null,$ip=null, $limit_pay = null, $timeStart = null,
