 ...
 * Class WxPayApi
 * @author shidatuo
 * @description 接口访问类，包含所有微信支付API列表的封装，类中方法为static方法，
...
class WxPayApi{
     ...
     *
     * 企业付款
     * 用于企业向微信用户个人付款目前支持向指定微信用户的openid付款
     * @param WxPayRefund $inputObj
     * @param int $timeOut
     * @throws WxPayException
     * @return 成功时返回，其他抛异常
    ...
    public static function transfers($inputObj, $timeOut = 6)
    {
        ...
        //检测必填参数
        $inputObj->mch_appid(WxPayConfig::$APPID);//公众账号ID
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        //运行curl
        $data = curl_exec($ch);
//        xn()->wechatpublic_manager->log_ex('postXmlCurl'.date('Y-m-d'), date('Y-m-d H:i:s') . "--- xml  -- \n"  .json_encode($xml) . PHP_EOL);
//        xn()->wechatpublic_manager->log_ex('postXmlCurl'.date('Y-m-d'), date('Y-m-d H:i:s') . "--- url  -- \n"  .json_encode($url) . PHP_EOL);
//        xn()->wechatpublic_manager->log_ex('postXmlCurl'.date('Y-m-d'), date('Y-m-d H:i:s') . "--- useCert  -- \n"  .json_encode($useCert) . PHP_EOL);
//        xn()->wechatpublic_manager->log_ex('postXmlCurl'.date('Y-m-d'), date('Y-m-d H:i:s') . "--- second  -- \n"  .json_encode($second) . PHP_EOL);
//        xn()->wechatpublic_manager->log_ex('postXmlCurl'.date('Y-m-d'), date('Y-m-d H:i:s') . "--- \$SSLCERT_PATH  -- \n"  .json_encode(WxPayConfig::$SSLCERT_PATH) . PHP_EOL);
//        xn()->wechatpublic_manager->log_ex('postXmlCurl'.date('Y-m-d'), date('Y-m-d H:i:s') . "--- \$SSLKEY_PATH  -- \n"  .json_encode(WxPayConfig::$SSLKEY_PATH) . PHP_EOL);
//        xn()->wechatpublic_manager->log_ex('postXmlCurl'.date('Y-m-d'), date('Y-m-d H:i:s') . "--- data  -- \n"  .json_encode($data) . PHP_EOL);
        //返回结果
        if ($data) {
            curl_close($ch);
            return $data;
        } else {
            //dump(WxPayConfig::$SSLCERT_PATH,$xml,$url);
            $error = curl_error($ch);
//            xn()->wechatpublic_manager->log_ex('test_refundOrderss'.date('Y-m-d'), date('Y-m-d H:i:s') . "--- error  -- \n"  .json_encode($error) . PHP_EOL);
            curl_close($ch);
